...
use bitnet_cpp_sys::ggml_log_level;
...
impl LlamaBackend {
    ...
    pub fn init() -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe { bitnet_cpp_sys::llama_backend_init() }
        Ok(LlamaBackend {})
    }
    ...
    pub fn init_numa(strategy: NumaStrategy) -> crate::Result<LlamaBackend> {
        Self::mark_init()?;
        unsafe {
            bitnet_cpp_sys::llama_numa_init(bitnet_cpp_sys::ggml_numa_strategy::from(strategy));
        }
        Ok(LlamaBackend {})
    }
    ...
    pub fn void_logs(&mut self) {
        ...
        unsafe {
            bitnet_cpp_sys::llama_log_set(Some(void_log), std::ptr::null_mut());
        }
    }
}
...
pub struct InvalidNumaStrategy(
    ...
    pub bitnet_cpp_sys::ggml_numa_strategy,
);

impl TryFrom<bitnet_cpp_sys::ggml_numa_strategy> for NumaStrategy {
    type Error = InvalidNumaStrategy;

    fn try_from(value: bitnet_cpp_sys::ggml_numa_strategy) -> Result<Self, Self::Error> {
        match value {
            bitnet_cpp_sys::GGML_NUMA_STRATEGY_DISABLED => Ok(Self::DISABLED),
            bitnet_cpp_sys::GGML_NUMA_STRATEGY_DISTRIBUTE => Ok(Self::DISTRIBUTE),
            bitnet_cpp_sys::GGML_NUMA_STRATEGY_ISOLATE => Ok(Self::ISOLATE),
            bitnet_cpp_sys::GGML_NUMA_STRATEGY_NUMACTL => Ok(Self::NUMACTL),
            bitnet_cpp_sys::GGML_NUMA_STRATEGY_MIRROR => Ok(Self::MIRROR),
            bitnet_cpp_sys::GGML_NUMA_STRATEGY_COUNT => Ok(Self::COUNT),
            value => Err(InvalidNumaStrategy(value)),
        }
    }
}

impl From<NumaStrategy> for bitnet_cpp_sys::ggml_numa_strategy {
    fn from(value: NumaStrategy) -> Self {
        match value {
            NumaStrategy::DISABLED => bitnet_cpp_sys::GGML_NUMA_STRATEGY_DISABLED,
            NumaStrategy::DISTRIBUTE => bitnet_cpp_sys::GGML_NUMA_STRATEGY_DISTRIBUTE,
            NumaStrategy::ISOLATE => bitnet_cpp_sys::GGML_NUMA_STRATEGY_ISOLATE,
            NumaStrategy::NUMACTL => bitnet_cpp_sys::GGML_NUMA_STRATEGY_NUMACTL,
            NumaStrategy::MIRROR => bitnet_cpp_sys::GGML_NUMA_STRATEGY_MIRROR,
            NumaStrategy::COUNT => bitnet_cpp_sys::GGML_NUMA_STRATEGY_COUNT,
        }
    }
}
...
impl Drop for LlamaBackend {
    fn drop(&mut self) {
        ...
        unsafe { bitnet_cpp_sys::llama_backend_free() }
    }
}
...
mod tests {
    ...
    fn numa_from_and_to() {
        ...
        for numa in &numas {
            let from = bitnet_cpp_sys::ggml_numa_strategy::from(*numa);
