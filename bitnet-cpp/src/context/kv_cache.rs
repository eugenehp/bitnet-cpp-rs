...
impl LlamaContext<'_> {
    ...
    pub fn copy_cache(&mut self, src: i32, dest: i32, size: i32) {
        unsafe { bitnet_cpp_sys::llama_kv_cache_seq_cp(self.context.as_ptr(), src, dest, 0, size) }
    }
    ...
    pub fn copy_kv_cache_seq(
        ...
        unsafe {
            bitnet_cpp_sys::llama_kv_cache_seq_cp(self.context.as_ptr(), src, dest, p0, p1);
        }
        Ok(())
    }
    ...
    pub fn clear_kv_cache_seq(
        ...
        let p1 = p1
            .map_or(Ok(-1), i32::try_from)
            .map_err(|e| KvCacheConversionError::P1TooLarge(e))?;
        Ok(unsafe { bitnet_cpp_sys::llama_kv_cache_seq_rm(self.context.as_ptr(), src, p0, p1) })
    }

    /// Returns the number of used KV cells (i.e. have at least one sequence assigned to them)
    #[must_use]
    pub fn get_kv_cache_used_cells(&self) -> i32 {
        unsafe { bitnet_cpp_sys::llama_get_kv_cache_used_cells(self.context.as_ptr()) }
    }

    /// Clear the KV cache
    pub fn clear_kv_cache(&mut self) {
        unsafe { bitnet_cpp_sys::llama_kv_cache_clear(self.context.as_ptr()) }
    }
    ...
    pub fn llama_kv_cache_seq_keep(&mut self, seq_id: i32) {
        unsafe { bitnet_cpp_sys::llama_kv_cache_seq_keep(self.context.as_ptr(), seq_id) }
    }
    ...
    pub fn kv_cache_seq_add(
        ...
        let p1 = p1
            ...
            .map_err(|e| KvCacheConversionError::P1TooLarge(e))?;
        unsafe {
            bitnet_cpp_sys::llama_kv_cache_seq_add(self.context.as_ptr(), seq_id, p0, p1, delta);
        }
        Ok(())
    }
    ...
    pub fn kv_cache_seq_div(
        ...
        unsafe { bitnet_cpp_sys::llama_kv_cache_seq_div(self.context.as_ptr(), seq_id, p0, p1, d) }
        Ok(())
    }
    ...
    pub fn kv_cache_seq_pos_max(&self, seq_id: i32) -> i32 {
        unsafe { bitnet_cpp_sys::llama_kv_cache_seq_pos_max(self.context.as_ptr(), seq_id) }
    }
    ...
    pub fn kv_cache_defrag(&mut self) {
        unsafe { bitnet_cpp_sys::llama_kv_cache_defrag(self.context.as_ptr()) }
    }

    /// Apply the KV cache updates (such as K-shifts, defragmentation, etc.)
    pub fn kv_cache_update(&mut self) {
        unsafe { bitnet_cpp_sys::llama_kv_cache_update(self.context.as_ptr()) }
    }
    ...
    pub fn get_kv_cache_token_count(&self) -> i32 {
        unsafe { bitnet_cpp_sys::llama_get_kv_cache_token_count(self.context.as_ptr()) }
    }
    ...
    pub fn new_kv_cache_view(&self, n_max_seq: i32) -> KVCacheView {
        let view =
            unsafe { bitnet_cpp_sys::llama_kv_cache_view_init(self.context.as_ptr(), n_max_seq) };
...
pub struct KVCacheViewCell {
    ...
    pub pos: bitnet_cpp_sys::llama_pos,
}
...
pub struct KVCacheView<'a> {
    ...
    view: bitnet_cpp_sys::llama_kv_cache_view,
}
...
impl<'a> KVCacheView<'a> {
    ...
    pub fn update(&mut self) {
        unsafe {
            bitnet_cpp_sys::llama_kv_cache_view_update(self.ctx.context.as_ptr(), &mut self.view);
        }
    }
    ...
    pub fn cells_sequences(&self) -> impl Iterator<Item = &[bitnet_cpp_sys::llama_seq_id]> {
        unsafe {
            std::slice::from_raw_parts(
...
impl<'a> Drop for KVCacheView<'a> {
    fn drop(&mut self) {
        unsafe {
            bitnet_cpp_sys::llama_kv_cache_view_free(&mut self.view);
        }
    }
}
