...
impl<'model> LlamaContext<'model> {
    pub(crate) fn new(
        ...
        llama_context: NonNull<bitnet_cpp_sys::llama_context>,
        embeddings_enabled: bool,
    ) -> Self {
        Self {
    ...
    pub fn n_batch(&self) -> u32 {
        unsafe { bitnet_cpp_sys::llama_n_batch(self.context.as_ptr()) }
    }

    /// Gets the max number of physical tokens (hardware level) to decode in batch. Must be less than or equal to n_batch.
    #[must_use]
    pub fn n_ubatch(&self) -> u32 {
        unsafe { bitnet_cpp_sys::llama_n_ubatch(self.context.as_ptr()) }
    }

    /// Gets the size of the context.
    #[must_use]
    pub fn n_ctx(&self) -> u32 {
        unsafe { bitnet_cpp_sys::llama_n_ctx(self.context.as_ptr()) }
    }
    ...
    pub fn decode(&mut self, batch: &mut LlamaBatch) -> Result<(), DecodeError> {
        let result =
            unsafe { bitnet_cpp_sys::llama_decode(self.context.as_ptr(), batch.llama_batch) };
    ...
    pub fn encode(&mut self, batch: &mut LlamaBatch) -> Result<(), EncodeError> {
        let result =
            unsafe { bitnet_cpp_sys::llama_encode(self.context.as_ptr(), batch.llama_batch) };
    ...
    /// - If the current model had a pooling type of [`bitnet_cpp_sys::LLAMA_POOLING_TYPE_NONE`]
    ...
    pub fn embeddings_seq_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = bitnet_cpp_sys::llama_get_embeddings_seq(self.context.as_ptr(), i);
    ...
    pub fn embeddings_ith(&self, i: i32) -> Result<&[f32], EmbeddingsError> {
        ...
        unsafe {
            let embedding = bitnet_cpp_sys::llama_get_embeddings_ith(self.context.as_ptr(), i);
    ...
    pub fn get_logits(&self) -> &[f32] {
        let data = unsafe { bitnet_cpp_sys::llama_get_logits(self.context.as_ptr()) };
    ...
    pub fn get_logits_ith(&self, i: i32) -> &[f32] {
        ...
        let data = unsafe { bitnet_cpp_sys::llama_get_logits_ith(self.context.as_ptr(), i) };
        let len = usize::try_from(self.model.n_vocab()).expect("n_vocab does not fit into a usize");
    ...
    //     unsafe { bitnet_cpp_sys::llama_reset_timings(self.context.as_ptr()) }
    // }

    /// Returns the timings for the context.
    // pub fn timings(&mut self) -> LlamaTimings {
    //     let timings = unsafe { bitnet_cpp_sys::llama_get_timings(self.context.as_ptr()) };
    ...
    pub fn lora_adapter_set(
        ...
        let err_code = unsafe {
            bitnet_cpp_sys::llama_lora_adapter_set(
                self.context.as_ptr(),
    ...
    pub fn lora_adapter_remove(
        ...
        let err_code = unsafe {
            bitnet_cpp_sys::llama_lora_adapter_remove(
                self.context.as_ptr(),
                adapter.lora_adapter.as_ptr(),
            )
        };
...
impl Drop for LlamaContext<'_> {
    fn drop(&mut self) {
        unsafe { bitnet_cpp_sys::llama_free(self.context.as_ptr()) }
    }
}
